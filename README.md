...
```
For the human feedback prediction tasks, we specify `min_score_gap` and `min_rank_gap` to only validate on less-noisy samples (not applied to training, which uses all samples).
```
...
```
For `human_vs_rand` task, use the `mismatch` flag
```
python src/main.py train --data=data/out/updown -p=restore/medium_ft.pkl --min_score_gap=20 --min_rank_gap=0.5 --mismatch
```
...
```
python src/main.py train --data=data/out/human_vs_machine -p=restore/medium_ft.pkl --min_score_gap=20 --min_rank_gap=0.5
```
...
To run locally, use the following command to test a single model (e.g. `updown.pth`)
```bash
